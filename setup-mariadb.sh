...
docker exec -i docker-xammp-ws-db-1 mariadb -u root -proot <<EOF
...
docker exec -i docker-xammp-ws-db-1 mariadb -u root -proot <./db/dumps/schema.sql
echo "DB schema loaded."

docker exec -i docker-xammp-ws-db-1 mariadb -u root -proot <./db/dumps/data.sql
echo "DB populated."